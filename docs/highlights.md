...
Le serveur prépare des extraits exacts des champs transmis, de 600 caractères au maximum, avec un identifiant et les nombres présents dans chaque extrait. Le modèle choisit les identifiants des extraits utilisés pour le **titre comme pour chaque paragraphe** ; le schéma JSON strict limite ces références à la liste réellement fournie. Il n’a donc plus à recopier des citations au caractère près. Le serveur vérifie les identifiants et n’accepte les nombres d’un bloc que s’ils figurent dans ses propres références. La détection des nombres écrits en lettres respecte les limites des mots Unicode : « récent » ne devient pas « cent ». Ces références internes ne sont ni publiées ni ajoutées aux articles enregistrés.

Le système rejette notamment les références inexistantes, les nouveaux chiffres détectables, les coordonnées dans le texte IA, les réponses trop longues, les balises et les réponses tronquées. Les longs passages recopiés du parcours (24 mots consécutifs) et les phrases conservées à la première personne sont également refusés. Les instructions contenues dans les profils et les extraits sont traitées comme des données non fiables. La sortie est affichée comme du texte, jamais comme du HTML. Le changement de format des références ne nécessite aucune migration SQL et conserve un seul appel par profil.
...
| `reason: invalid_evidence` | Une référence est absente du catalogue fourni au modèle, ou son format est invalide. Les versions antérieures exigeaient une citation exacte rédigée par le modèle. |
| `reason: unsupported_number` | Un nombre du titre ou d’un paragraphe n’est pas présent dans les extraits référencés par ce bloc. |
