...
Le job appelle `mistral-small-latest` une fois pour chaque profil, avec un maximum de 1 500 tokens de sortie et un délai de 45 secondes. `MISTRAL_MODEL` permet de fixer une version différente. Les seuls champs transmis sont le nom, la promotion, les domaines/spécialités, la localisation, le parcours et le mentorat déclaré, avec des longueurs limitées. Les coordonnées de la table de contacts, le genre, l’identifiant, la photo et les données d’authentification ne sont pas transmis. Les champs libres restent les déclarations du membre et peuvent contenir les informations qu’il y a lui-même saisies.

Le titre est construit à partir du nom. Pour les paragraphes, le système impose un JSON structuré et exige des citations exactes des champs utilisés. Il rejette notamment les références inexistantes, les nouveaux chiffres détectables, les coordonnées dans le texte IA, les réponses trop longues, les balises et les réponses tronquées. Les instructions contenues dans les profils sont traitées comme des données non fiables. La sortie est affichée comme du texte, jamais comme du HTML.
...
En cas d’échec, un texte de secours est composé automatiquement à partir des champs renseignés et d’un extrait explicitement cité du parcours. Il est enregistré avec `generation_method = 'fallback'` et identifié comme tel. Le job ne relance pas Mistral pour cette édition, même après correction de la clé. Il ne faut pas effacer les marqueurs de tentative pour forcer une nouvelle génération.
...
Il y a au maximum deux requêtes Mistral par édition, soit généralement 8 à 10 par mois. Les lectures publiques utilisent une projection explicite des champs et `Cache-Control: no-store`, afin qu’un profil retiré ne reste pas affiché dans un cache intermédiaire. Ce choix n’occasionne aucun nouvel appel IA.

Le Blueprint Render prépare un job séparé, indépendant de la veille de l’API gratuite, le lundi à 00 h puis à 00 h 20. Le second passage reprend uniquement une éventuelle interruption ; une édition publiée ne fait aucun travail supplémentaire. Si les deux passages échouent à accéder à la base, il faut relancer le job manuellement. Les logs indiquent `published`, `busy`, `empty` et le nombre de textes de secours, sans afficher de secrets ni les contenus des profils.
...
1. Appliquer [la migration Highlights](../supabase/migrations/202609050001_weekly_highlights.sql), après la migration initiale, dans l’environnement Supabase visé. Elle conserve les profils existants et laisse leur genre non renseigné.
...
Les tests incluent un PostgreSQL embarqué PGlite qui exécute les deux migrations dans une base éphémère, des simulations du fournisseur Mistral et les cas de reprise/concurrence. Ils ne lisent pas la base configurée et n’appellent pas Mistral. L’émulation PostgreSQL vérifie les fonctions et permissions ; elle ne remplace pas un essai de concurrence entre plusieurs connexions ni un test du proxy REST Supabase en environnement d’essai.
